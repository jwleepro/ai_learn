...
> 소스: [`train_bigram_nn.py`](code/train_bigram_nn.py)
```powershell
python code/train_bigram_nn.py --input data/tiny_corpus_ko.txt --epochs 40 --lr 2.0 --batch 2048
```
...
> 소스: [`generate_bigram_nn.py`](code/generate_bigram_nn.py)
```powershell
python code/generate_bigram_nn.py --model llm_from_scratch/models/bigram_nn.npz --length 300 --seed 0 --temperature 1.0
```
...
`data/my_corpus.txt`를 만들고 (200~5000자),
같은 방식으로 학습/생성을 해보세요.
