...
토큰 ID는 단지 "번호표"에 가깝습니다.
예를 들어 id가 `7`이라고 해서 id `3`보다 의미가 2배 크다는 뜻은 아닙니다.
그런 숫자를 그대로 넣으면 모델이 없는 크기 관계를 읽어버릴 수 있으므로, 학습 가능한 벡터로 바꿉니다.
...
- 비슷한 역할의 토큰은 비슷한 방향의 벡터로 배울 수 있고
- 뒤의 선형층/MLP가 토큰을 연속적인 특징 공간에서 다룰 수 있습니다
...
핵심은, 빅램처럼 직전 1개만 보지 않고 최근 `k`개 토큰을 한 번에 묶어
"이 문맥이면 다음 토큰이 무엇일 가능성이 큰가"를 점수화한다는 점입니다.
아직 attention처럼 위치별로 고르는 능력은 없지만, "고정 길이 문맥을 보는 첫 모델"로는 충분합니다.
