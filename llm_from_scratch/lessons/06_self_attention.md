...
attention을 기호 없이 말하면 아래 4단계입니다.

1. 각 위치에서 "내가 지금 무엇을 찾는지"를 나타내는 표를 만듭니다. (`Q`)
2. 각 위치에서 "내가 가진 단서"와 "실제 내용" 표를 만듭니다. (`K`, `V`)
3. 현재 위치의 `Q`와 다른 위치의 `K`를 비교해, 누구를 얼마나 볼지 점수를 매깁니다.
4. 그 점수를 비율로 바꾼 뒤, `V`를 그 비율만큼 섞습니다.

즉, **"누구를 얼마나 참고할지 정하고, 그 비율로 정보를 섞는 계산"** 입니다.
굳이 `Q`, `K`, `V`를 나누는 이유는 "무엇을 찾는가", "무슨 단서를 갖고 있는가", "실제로 가져올 내용이 무엇인가"를
같은 벡터 하나에 억지로 몰아넣지 않기 위해서입니다.
그래야 "매칭은 Q-K로 하고, 실제 전달할 정보는 V로 한다"는 역할 분리가 가능합니다.
...
- 입력 임베딩: `X` (T x D)
- `Q = XWq`, `K = XWk`, `V = XWv`
- 점수: `scores = Q @ K^T / sqrt(Dh)`
- 가중치: `weights = softmax(scores)`
- 출력: `out = weights @ V` (가중합)

여기서:

- `K^T`는 K의 행과 열을 뒤집은 것(전치)입니다. 코드에서는 `K.T`로 씁니다.
- `/ sqrt(Dh)`는 **head 차원이 커질수록 점수가 너무 커지지 않게 나누는 안전장치**입니다.
- 공식을 외울 필요는 없고, "Q와 K를 비교해 비율을 만들고 V를 섞는다"만 기억하면 됩니다.
