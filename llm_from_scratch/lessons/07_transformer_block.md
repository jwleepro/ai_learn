...
- 깊은 네트워크에서도 정보가 한 번에 사라지지 않아 학습이 안정적
- 처음부터 표현을 새로 만들기보다, 필요한 수정만 덧붙이는 형태라 바꾸기 쉬움
...
이렇게 하는 이유는 attention/MLP를 지나며 값의 스케일이 크게 흔들리면 다음 층이 다루기 어려워지기 때문입니다.
먼저 스케일을 정리해 두고, 필요한 표현력은 학습 가능한 스케일/시프트가 다시 보충합니다.
